...
//! Portable across x86 (AVX2) and ARM (NEON) via intrinsics (stable Rust).
...
pub fn divmod_batch<const N: usize>(vec: [u32; N]) -> ([u32; N], [u8; N]) {
    ...
    for lane in 0..N {
        ...
        let q = ((wide >> 32) as u32).wrapping_add(carry >> 32);  // Approx quot
        ...
        carry = quot[lane] >> 32;  // Rare high bit
    }
    (quot, rem)
}

/// Unrolled Horner for decode: sum (acc * BASE + val * BASE^i) but per-batch.
/// Actually: sum_{j=0}^{N-1} val_j * powers[j] (then * BASE^N for cascade).
/// u64 lanes: No overflow (58^8 ~1e14 * 57 < 2^64).
/// Fixed N: Loop unrolls fully.
#[inline(always)]
...
mod tests {
    ...
    fn test_divmod_correction() {
        // Case needing +1: 57*58 + 57 = 3364-1? Test edge 3359 /58 =57 rem53
